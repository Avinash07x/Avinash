...
//         <h2 className="text-6xl font-bold  bg-blue-500 bg-clip-text text-transparent">
//           Featured Projects
//         </h2>
//       </div>
...
const Projects = () => {
  ...
  // ✅ Improved 3D Positioning Logic (STRONGER LEFT/RIGHT + BIGGER IMAGE)
  const updateCarousel = (index) => {
    const items = carouselRef.current?.children;
    if (!items) return;
    ...
    Array.from(items).forEach((item, i) => {
      ...
      const translateX = diff * 420; // ✅ Stronger slide
  ...
  return (
    <section className="min-h-screen bg-gray-900 py-20 px-4 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto bg-gray-800 p-8 rounded-xl text-white">
        <h3
          className={`text-3xl font-bold bg-gradient-to-r ${currentProject.color} bg-clip-text text-transparent mb-4`}
        >
          {currentProject.title}
        </h3>
